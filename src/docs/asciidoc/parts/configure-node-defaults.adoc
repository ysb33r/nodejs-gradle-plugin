...
If no executable is configured, then Gradle will attempt to download the version specified in links:{api}/org/ysb33r/gradle/nodejs/NodeJSExtension.html#NODEJS_DEFAULT[NodeJSExtension.NODEJS_DEFAULT]. If Gradle runs on a <<platforms,non-supported platform>> this will fail.
