//
// ============================================================================
// (C) Copyright Schalk W. Cronje 2017
//
// This software is licensed under the Apache License 2.0
// See http://www.apache.org/licenses/LICENSE-2.0 for license details
//
// Unless required by applicable law or agreed to in writing, software distributed under the License is
// distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and limitations under the License.
//
// ============================================================================
//

package org.ysb33r.gradle.nodejs.impl

import groovy.transform.CompileDynamic
import groovy.transform.CompileStatic
import org.gradle.api.Project
import org.ysb33r.gradle.olifant.StringUtils
import org.ysb33r.gradle.olifant.exec.ResolvedExecutable

/** Runs NPM
 *
 * @since 0.1
 */
@CompileStatic
class NpmExecutor {

    NpmExecutor(Project project) {

    }

    /** Configure this exeutor using a closure
     *
     * @param cfg
     * @return
     */
    NpmExecutor configure(Closure cfg) {
        Closure configurator = (Closure)(cfg.clone())
        configurator.delegate = this
        configurator.resolveStrategy = Closure.DELEGATE_FIRST
        configurator()
        return this
    }


    void command(final Object cmd) {
        this.command = cmd
    }

    void setCommand(final Object cmd) {
        command(cmd)
    }

    String getCommand() {
        StringUtils.stringize(this.command)
    }

    @CompileDynamic
    def run() {
        project.exec {
            executable npm.getExecutable()
            args getCommand()
        }
    }

    private ResolvedExecutable getNode() {
        null
    }

    private ResolvedExecutable getNpm() {
        null
    }


    private Project project
    private Object command
}
